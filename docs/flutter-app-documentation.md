...
| Structured local storage | **Isar** | Theme objects are structured documents; Isar is fast, null-safe, no codegen-heavy migrations, good query API. Hive is in maintenance mode; drift is overkill (no relational queries needed). |
...
| Serialization | **freezed + json_serializable** | Immutable models, unions for state machines (entitlement states, overlay states). |
...
| Crash reporting | **Sentry (sentry_flutter)** | Captures Dart + Kotlin (NDK) crashes, breadcrumbs, release health. Firebase Crashlytics is fine too, but Sentry self-hosting is possible later and it pairs with the backend's Sentry. |
